...
Untrusted input is a chargeback dispute; it can only ever become one of a fixed set of commands,
...
- **Untrusted input:** `fetch<web>` — a chargeback dispute
- **Closed type:** `type Reason = Fraud | NotReceived | Duplicate | Unauthorized`, then `type Decision = Accept | Contest(Reason) | Escalate` (a payload-carrying variant — the payload is closed too)
- **Sink / capability:** `grant handle` → `handle(d)`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/chargeback-handler/demo.sh
```
...
- `chargeback-handler_safe.wave` — the correct design.
- `chargeback-handler_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
